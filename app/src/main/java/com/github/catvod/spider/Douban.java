...
public class Douban extends Spider {
    ...
    public String homeContent(boolean filter) throws Exception {
        ...
        return Result.get().classes(classes).filters(filterConfig).string();
    }
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) throws Exception {
        ...
        String tags = URLEncoder.encode(getTags(ext), "UTF-8");
        ...
        return Result.get().page(Integer.parseInt(pg), 999, 20, 999 * 20).vod(videos).string();
    }

    private String getRating(JSONObject item) {
        try {
            if (item.has("rating") && !item.isNull("rating")) {
    ...
    private String getPic(JSONObject item) {
        try {
            if (item.has("pic") && !item.isNull("pic")) {
    ...
    public String detailContent(List<String> ids) throws Exception {
        ...
        Vod vodInfo = new Vod();
        vodInfo.setVodId(ids.get(0));
        vodInfo.setVodName(name);
        vodInfo.setVodPic(pic);
        vodInfo.setTypeName("豆瓣数据源");
        vodInfo.setVodActor(tips);
        vodInfo.setVodDirector(tips);
        vodInfo.setVodContent(tips);

        return Result.string(vodInfo);
    }

    @Override
    public String searchContent(String keyword, boolean quick) throws Exception {
        if (TextUtils.isEmpty(keyword)) return Result.get().string();
        
        String searchURL = hostURL + "/search/subjects?q=" + URLEncoder.encode(keyword, "UTF-8") + "&apikey=" + apikey + "&start=0&count=20";
        String jsonStr = OkHttp.string(searchURL, getHeader());
